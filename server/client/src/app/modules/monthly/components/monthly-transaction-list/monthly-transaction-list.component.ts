        ...
        categories.forEach(category => {
          if (category.transactionsCount > 0) {
            this.monthlyCategories[category._id] = [
              { name: category.name, exp: 0, inc: 0, items: [] }
            ];
          }

          transactions.forEach(transaction => {
            if (transaction._categoryId === category._id) {
              this.monthlyCategories[transaction._categoryId][0].items.push(
                transaction
              );

              if (transaction.type === 'exp') {
                this.monthlyCategories[transaction._categoryId][0].exp +=
                  transaction.value;
              }
              if (transaction.type === 'inc') {
                this.monthlyCategories[transaction._categoryId][0].inc +=
                  transaction.value;
